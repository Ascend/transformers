...
MODEL_NAME_OR_PATH = "facebook/levit-128S"
...
class LevitModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        return LevitImageProcessor.from_pretrained(MODEL_NAME_OR_PATH)

    def test_inference_image_classification_head(self):
        model = LevitForImageClassificationWithTeacher.from_pretrained(MODEL_NAME_OR_PATH).to(
            torch_device
        )
...
if __name__=='__main__':
    unittest.main(argv=['', 'LevitModelIntegrationTest.test_inference_image_classification_head'])

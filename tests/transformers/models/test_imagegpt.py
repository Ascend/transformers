...
""" Testing suite for the PyTorch Imagegpt model. """
...
MODEL_NAME_OR_PATH = "openai/imagegpt-small"
...
class ImageGPTModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        ...
        return ImageGPTImageProcessor.from_pretrained(MODEL_NAME_OR_PATH) if is_vision_available() else None

    def test_inference_causal_lm_head(self):

        model = ImageGPTForCausalImageModeling.from_pretrained(MODEL_NAME_OR_PATH).to(torch_device)
...
if __name__=='__main__':
    unittest.main(argv=['', 'ImageGPTModelIntegrationTest.test_inference_causal_lm_head'])

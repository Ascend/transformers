...
from optimum.ascend import NPUTrainingArguments
from optimu.utils import logging
...
if is_torch_available():
    ...
    from optimum.ascend import NPUTrainer
...
class RegressionNPUTrainingArguments(NPUTrainingArguments):
    a: float = 0.0
    b: float = 0.0
...
if is_torch_available():
    ...
    class CustomDataloaderTrainer(NPUTrainer):
    ...
    def get_regression_trainer(a=0, b=0, double_output=False, train_len=64, eval_len=64, pretrained=True, **kwargs):
        ...
        args = RegressionNPUTrainingArguments(output_dir, a=a, b=b, use_ascend=True, npu_fp16=True, **kwargs)
        return NPUTrainer(
            model,
            args,
...
class NPUTrainerIntegrationCommon:
...
@require_sentencepiece
@require_tokenizers
class NPUTrainerIntegrationPrerunTest(TestCasePlus, NPUTrainerIntegrationCommon):
    """
    ...
    def setUp(self):
        super().setUp()
        args = NPUTrainingArguments("..", use_ascend=True, npu_fp16=True)
    ...
    def test_trainer_with_datasets(self):
        ...
        args = NPUTrainingArguments("./regression", learning_rate=0.1, use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(model, args, train_dataset=train_dataset)
        trainer.train()
        self.check_trained_model(trainer.model)
    ...
    def test_model_init(self):
        ...
        args = NPUTrainingArguments("./regression", learning_rate=0.1, use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(args=args, train_dataset=train_dataset, model_init=lambda: RegressionModel())
        trainer.train()
        self.check_trained_model(trainer.model)
    ...
    def test_custom_optimizer(self):
        train_dataset = RegressionDataset()
        args = NPUTrainingArguments("./regression", use_ascend=True, npu_fp16=True)
        model = RegressionModel()
        optimizer = torch.optim.SGD(model.parameters(), lr=1.0)
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda x: 1.0)
        trainer = NPUTrainer(model, args, train_dataset=train_dataset, optimizers=(optimizer, lr_scheduler))
        trainer.train()
    ...
    def test_mixed_bf16(self):
        # --bf16 is not supported by NPU and should raise an error
        with self.assertRaises(ValueError):
            trainer = get_regression_trainer(learning_rate=0.1, bf16=True)

    def test_tf32(self):
        # --tf32 is not supported by NPU and should raise an error
        with self.assertRaises(ValueError):
...
@require_sentencepiece
@require_tokenizers
class NPUTrainerIntegrationTest(TestCasePlus, NPUTrainerIntegrationCommon):
    def setUp(self):
        super().setUp()
        args = NPUTrainingArguments("..", use_ascend=True, npu_fp16=True)
        self.n_epochs = args.num_train_epochs
    ...
    def test_trainer_works_with_dict(self):
        ...
        args = NPUTrainingArguments("./regression", use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(model, args, train_dataset=train_dataset, eval_dataset=eval_dataset)
        trainer.train()
        _ = trainer.evaluate()
    ...
    def test_evaluation_with_keys_to_drop(self):
        ...
        args = NPUTrainingArguments("./test", use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(tiny_gpt2, args, eval_dataset=eval_dataset)
    ...
    def test_training_arguments_are_left_untouched(self):
        trainer = get_regression_trainer()
        trainer.train()
        args = NPUTrainingArguments("./regression", use_ascend=True, npu_fp16=True, report_to=[])
    ...
    # inf and nan are not supported by NPU
    def test_logging_inf_nan_filter(self):
        ...
        args = NPUTrainingArguments("./test", use_ascend=True, npu_fp16=True,
                                    learning_rate=1e9, logging_steps=5, logging_nan_inf_filter=False)
        trainer = NPUTrainer(tiny_gpt2, args, train_dataset=train_dataset)
        trainer.train()
        ...
        self.assertFalse(is_any_loss_nan_or_inf(log_history_no_filter))
    ...
    def test_dataloader_without_dataset(self):
        ...
        args = NPUTrainingArguments(output_dir="tmp_trainer", use_ascend=True, npu_fp16=True)
        trainer = CustomDataloaderTrainer(
            model=RegressionModel(), args=args, train_dataset=train_dataset, eval_dataset=train_dataset
        )
        trainer.train()
        trainer.evaluate()
    ...
    def test_data_is_not_parallelized_when_model_is_parallel(self):
        ...
        trainer = NPUTrainer(model, args, train_dataset=RegressionDataset(), eval_dataset=RegressionDataset())
    ...
    # # TODO: jit is not supported by NPU
    ...
    # # TODO: dynamic shape is not supported by NPU
    ...
    def test_resume_training_with_randomness(self):
        ...
        with self.subTest("Test every step"):
            ...
            args = RegressionNPUTrainingArguments(tmp_dir, save_steps=5, learning_rate=0.1,
                                                  use_ascend=True, npu_fp16=True)
            trainer = NPUTrainer(model, args, train_dataset=train_dataset, eval_dataset=eval_dataset)
        ...
        with self.subTest("Test every epoch"):
            ...
            tmp_dir = self.get_auto_remove_tmp_dir()
            args = RegressionNPUTrainingArguments(tmp_dir, use_ascend=True, npu_fp16=True,
                                                  save_strategy="epoch", learning_rate=0.1)
            trainer = NPUTrainer(model, args, train_dataset=train_dataset, eval_dataset=eval_dataset)
    ...
    def test_training_with_resume_from_checkpoint_false(self):
        ...
        tmp_dir = self.get_auto_remove_tmp_dir()
        args = RegressionNPUTrainingArguments(tmp_dir, use_ascend=True, npu_fp16=True, save_steps=5, learning_rate=0.1)
        trainer = NPUTrainer(model, args, train_dataset=train_dataset, eval_dataset=eval_dataset)
    ...
    def test_training_iterable_dataset(self):
        ...
        args = RegressionNPUTrainingArguments(output_dir="./examples", max_steps=4, use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(model=model, args=args, train_dataset=train_dataset)
        trainer.train()
    ...
    def test_training_finite_iterable_dataset(self):
        ...
        train_args = NPUTrainingArguments(
            "..",
        ...
        trainer = NPUTrainer(model, train_dataset=data, args=train_args)
        with self.assertLogs("optimum.ascend.trainer", level="WARNING") as logs:
            trainer.train()
    ...
    def test_evaluation_iterable_dataset(self):
        ...
        args = RegressionNPUTrainingArguments(output_dir="./examples", use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(model=model, args=args, eval_dataset=eval_dataset, compute_metrics=AlmostAccuracy())
        results = trainer.evaluate()
    ...
    # # TODO: torchdynamo is not supported by NPU
    # @require_torch_non_multi_gpu
    # @require_torchdynamo
    # @require_torch_tensorrt_fx
    ...
    def test_no_wd_param_group(self):
        ...
        args = NPUTrainingArguments(output_dir="./test", use_ascend=True, npu_fp16=True)
        trainer = NPUTrainer(model=model, args=args)
        trainer.create_optimizer_and_scheduler(10)
        # fmt: off
        wd_names = ['0.linear1.weight', '0.linear2.weight', '1.0.linear1.weight', '1.0.linear2.weight',
                    '1.1.linear1.weight', '1.1.linear2.weight']
        # fmt: on
...
@require_torch
@require_optuna
class NPUTrainerHyperParameterOptunaIntegrationTest(unittest.TestCase):
    def setUp(self):
        args = NPUTrainingArguments("..", use_ascend=True, npu_fp16=True)
        self.n_epochs = args.num_train_epochs
        self.batch_size = args.train_batch_size

...
#################启动训练脚本#################
start_time=$(date +%s)
root_path=$(echo ${cur_path} | awk -F '/text-classification' '{print $1}')
nohup python3.7 -m torch.distributed.launch --nproc_per_node 8 run_glue.py \
  --model_name_or_path ${root_path}/model_path/${Network} \
  --task_name sst2 \
  --do_train \
  --do_eval \
  --num_train_epochs 3 \

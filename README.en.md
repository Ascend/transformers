...
- [NPUTrainer](https://gitee.com/ascend/transformers/blob/optimum/optimum/ascend/trainer.py): the trainer class that takes care of distributing the model to run on NPUs, and performing training and evaluation.
- [NPUTrainingArguments](https://gitee.com/ascend/transformers/blob/optimum/optimum/ascend/training_args.py): the class that enables to configure Ascend Mixed Precision and to decide whether optimized operators and optimizers should be used or not.

The [NPUTrainer](https://gitee.com/ascend/transformers/blob/optimum/optimum/ascend/trainer.py) is very similar to the 🤗 [Transformers Trainer](https://github.com/huggingface/transformers/blob/main/src/transformers/trainer.py), and adapting a script using the Trainer to make it work with NPU will mostly consist in simply swapping the `Trainer` class for the `NPUTrainer` one. That's how most of the [example scripts](https://gitee.com/ascend/transformers/blob/optimum/examples/question-answering/run_qa.py) were adapted from their [original counterparts](https://github.com/huggingface/transformers/blob/v4.25.1/examples/pytorch/question-answering/run_qa.py).

...
def npu_wav2vec2forpretraining_forward(
    ...
    """
    Copied from Wav2Vec2ForPreTraining:
        https://github.com/huggingface/transformers/blob/main/src/tranformers/models/wav2vec2/modeling_wav2vec2.py
    Due to 910 does not support inf/nan, the only difference is:
    ...
    if sampled_negative_indices is not None:
        ...
        contrastive_loss = nn.functional.cross_entropy(
            logits.to("cpu"), target.to("cpu"), reduction="sum"
        ).to(target.device)
